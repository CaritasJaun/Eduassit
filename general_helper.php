...
// birthdays today
if (!function_exists('get_today_birthdays')) {
    function get_today_birthdays($branch_id = null)
    {
        $CI =& get_instance();
        $CI->db->select('first_name, last_name, birthday');
        $CI->db->from('student');
        $CI->db->where('DATE_FORMAT(birthday, "%m-%d") =', date('m-d'));
...
if (!function_exists('to_lower')) {
    function to_lower($value): string
    {
        // Prevent "Passing null to parameter #1 ($string) of type string is deprecated"
        return strtolower((string)($value ?? ''));
    }
}

function get_loggedin_designation() {
    $CI =& get_instance();
    if (is_loggedin()) {
        $userID = get_loggedin_user_id();
        $designation = $CI->db->select('staff_designation.name')
            ->from('staff')
            ->join('staff_designation', 'staff.designation = staff_designation.id', 'left')
            ->where('staff.id', $userID)
            ->get()->row();

        return isset($designation->name) ? to_lower($designation->name) : null;
    }
    return null;
}
